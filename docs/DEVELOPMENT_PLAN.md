...
**状态：阶段一“应用窗口骨架”、阶段二“项目首页与项目级入口”、阶段三“创作工作区大框架”、阶段四“作品、资料与文档阅读模块”和阶段五“聊天与 Tool 授权模块”已完成；后续页面迁移未开始。**

已完成的窗口级迁移：

- `App` 继续作为唯一的 `DesktopShell`，以 Tailwind utilities 建立标题栏、应用主体和状态栏的三行布局；不创建平行外壳或全局状态。
- `WindowTitlebar`、全局导航、功能区容器和状态栏已改用语义 token 与 Tailwind utilities；标题栏原有独立 CSS 已删除。
- 保持既有标题栏菜单、作品/资料/设置切换、项目标题和“就绪”状态，不增加新的可见入口或业务行为。
- 未打开项目时的项目首页已迁移至 Tailwind、Button 和 Input；保留初始 DeepSeek API Key 配置、新建项目、打开项目及其失败反馈的既有业务链路。
- 创作工作区已迁移为 Tailwind 三栏 Grid；作品/资料左栏、中间文档区和右侧聊天区只替换各自容器，继续保留既有共享文档与聊天实例。文档区不再为已删除的 `reader-footer` 保留第三行高度。

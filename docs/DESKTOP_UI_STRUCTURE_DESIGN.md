...
窗口骨架、项目首页、创作工作区外壳、作品/资料阅读、聊天及设置迁移，以及自动收尾验证均已完成：`App` 仍是唯一的 `DesktopShell`，标题栏、全局导航、功能区容器和状态栏已经使用语义 token 与 Tailwind utilities。未打开项目时的项目首页复用既有 Typed IPC，并使用 Button 与 Input 呈现已确认的 API Key 配置、新建项目和打开项目操作。创作工作区使用三栏 Grid 承载作品/资料左栏、共享文档区与共享聊天区，切换作品/资料时只替换左栏。作品目录、资料列表、资料操作、标签栏与 Markdown/TXT 纯文本阅读区已经迁移；文件读取、目录监听、资料操作、标签页和阅读滚动仍使用既有业务状态。聊天模块已迁移 Conversation 列表、消息流、Reasoning 展开、输入、发送与 Tool 授权菜单，继续复用原有 Conversation、流式消息与审批状态。设置页已迁移其分类导航、DeepSeek API 表单及主题选择，继续复用已有安全存储与主题状态；不得因为这些视觉迁移而改变既有业务结构。被替代的页面级手写样式和旧资源引用已经清理；`styles.css` 仅保留根节点尺寸、字体继承和窄窗口导航宽度基线；Tailwind Preflight 负责重置原生控件，并以语义 token 提供统一边框颜色。全量格式、静态检查、测试和生产构建已通过；安装版中的人工主题、视觉与交互检查仍在 v0.3 发布门执行。

...
当前已完成窗口骨架与项目首页迁移：`App` 仍是唯一的 `DesktopShell`，标题栏、全局导航、功能区容器和状态栏已经使用语义 token 与 Tailwind utilities。未打开项目时的项目首页复用既有 Typed IPC，并使用 Button 与 Input 呈现已确认的 API Key 配置、新建项目和打开项目操作。作品、资料、阅读、聊天与设置的内部模块尚未迁移，不得因为前两阶段迁移而提前调整其业务结构。

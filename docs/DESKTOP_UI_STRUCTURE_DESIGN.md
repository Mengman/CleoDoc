...
当前已完成窗口骨架、项目首页、创作工作区外壳及作品/资料阅读迁移：`App` 仍是唯一的 `DesktopShell`，标题栏、全局导航、功能区容器和状态栏已经使用语义 token 与 Tailwind utilities。未打开项目时的项目首页复用既有 Typed IPC，并使用 Button 与 Input 呈现已确认的 API Key 配置、新建项目和打开项目操作。创作工作区使用三栏 Grid 承载作品/资料左栏、共享文档区与共享聊天区，切换作品/资料时只替换左栏。作品目录、资料列表、资料操作、标签栏与 Markdown/TXT 纯文本阅读区已经迁移；文件读取、目录监听、资料操作、标签页和阅读滚动仍使用既有业务状态。聊天与设置的内部模块尚未迁移，不得因为前四阶段迁移而提前调整其业务结构。

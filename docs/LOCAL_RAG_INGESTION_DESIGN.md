...
运行时优先使用发行的预编译绑定。顶层软件配置 `gpuAcceleration` 是 CleoDoc 所有 GPU 能力共用的全局开关：关闭时，Windows、Linux 和 Intel macOS 选择 CPU 绑定；Apple Silicon 因发行包只提供 ARM64 Metal 绑定而选择 `metal`，但模型加载仍设置 `gpuLayers: 0`，不启用 GPU 模型层推理。开启时，完整 Embedding Runtime 与 `vocabOnly` Tokenizer 都向 `node-llama-cpp` 传入 `gpu: "auto"` 和 `gpuLayers: "auto"`，由运行库根据当前平台、可用预编译绑定和硬件自动选择。

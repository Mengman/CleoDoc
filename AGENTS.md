...
- 任何远程调用都应能够还原所用模型、参数和 `ContextManifest`。
...
- 当前数据库基线是 Schema v10；全新数据库直接创建当前结构，完整 v8 数据库依次执行 v8→v9→v10 前向迁移，完整 v9 数据库执行 v9→v10 前向迁移，v7 及更旧或高于当前版本的数据库必须拒绝且不得自动改写或删除。正式发布后的数据库 Schema 变化必须继续提供前向 migration。
...
3. 项目隔离和路径边界没有被绕过。
4. 新增索引和缓存可以从事实源重建。
5. LLM 使用的上下文可以通过 `ContextManifest` 审计。
